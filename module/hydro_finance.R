...
#' Convert daily flow volumes to daily hydroelectric energy generation
#'
#' Calculates the daily energy generated by a turbine from the daily flow
#' volume diverted to power and the effective hydraulic head, using the
#' standard hydropower physics relation \eqn{P = \rho g H Q \eta}.
#'
#' \deqn{E_{kWh} = \frac{\rho \, g \, H \, Q \, \eta \times 86400}{3.6 \times 10^6}}
#'
#' where \eqn{Q} is the flow rate in m\eqn{^3}/s (obtained by converting the
#' daily flow volume from m\eqn{^3}/day), \eqn{H} is the hydraulic head (m),
#' \eqn{\eta} is the turbine-generator efficiency, and the energy is converted
#' from joules to kWh by dividing by \eqn{3.6 \times 10^6}.
#'
#' @param power_flows Numeric vector. Daily flow volume diverted to the
#'   turbine (m\eqn{^3}/day), e.g. \code{Q_power_cms * 86400}.
#' @param height Numeric. Effective hydraulic head (m).
#' @param rho Numeric. Water density (kg/m\eqn{^3}). Default \code{1000}.
#' @param g Numeric. Gravitational acceleration (m/s\eqn{^2}). Default \code{9.8}.
#' @param Keff Numeric. Turbine-generator efficiency (dimensionless, 0-1).
#'   Default \code{0.8}.
#'
#' @return Numeric vector, the same length as \code{power_flows}, giving the
#'   energy generated each day (kWh/day).
#'
#' @examples
#' \dontrun{
#' # 100 days at a constant 10 m^3/s diverted through a 50 m head turbine
#' daily_volume_m3 <- rep(10 * 86400, 100)
#' daily_kwh <- energy_production(daily_volume_m3, height = 50)
#' sum(daily_kwh) / 1e3   # total generation in MWh over the period
#' }
#'
#' @export
